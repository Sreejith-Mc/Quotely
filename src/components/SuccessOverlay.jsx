export default function SuccessOverlay({ open, number, grandTotal, updated, onClose, onNewQuote }) {
  if (!open) return null;
  return (
    <div onClick={onClose} style={{ position: 'fixed', inset: 0, zIndex: 70, background: 'rgba(12,16,14,0.55)', display: 'flex', alignItems: 'center', justifyContent: 'center', animation: 'fadeIn .14s', backdropFilter: 'blur(3px)', padding: 16 }}>
      <div onClick={(e) => e.stopPropagation()} style={{ width: '100%', maxWidth: 380, background: 'var(--panel)', borderRadius: 20, padding: 32, textAlign: 'center', boxShadow: '0 30px 80px rgba(0,0,0,0.3)' }}>
        ...
        <div style={{ font: '800 19px Manrope', color: 'var(--ink)' }}>{updated ? 'Quotation Updated' : 'Quotation Generated'}</div>
        <p style={{ font: '500 13px Manrope', color: 'var(--ink-2)', margin: '6px 0 2px' }}>{number} has been {updated ? 'updated' : 'created and saved'}.</p>
        ...
        <div style={{ display: 'flex', gap: 10 }}>
          ...
          <button onClick={onNewQuote} style={{ flex: 1, border: 'none', background: 'var(--green)', color: '#fff', cursor: 'pointer', font: '700 13px Manrope', padding: 11, borderRadius: 11 }}>{updated ? 'Back to Dashboard' : 'New Quotation'}</button>
        </div>
      </div>
    </div>
  );
